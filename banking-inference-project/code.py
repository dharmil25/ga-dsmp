...
axes[i].hist(mean_series, bins=8)
plt.show()
...
# remove the % character and convert the column into float
data['int.rate'] = data['int.rate'].map(lambda x: x.strip('%')).astype(float)

# divide the values of int.rate with 100
